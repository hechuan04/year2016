...
- (void)initView {
    ...
    self.largeFrame = CGRectMake(oriX-self.limitRatio/2*self.oldFrame.size.width,oriY-self.limitRatio/2*self.oldFrame.size.height, self.limitRatio * self.oldFrame.size.width, self.limitRatio * self.oldFrame.size.height);
...
- (void)initControlBtn {
    ...
    cancelBtn.backgroundColor = [UIColor colorWithWhite:0 alpha:.5];
    ...
    confirmBtn.backgroundColor = [UIColor colorWithWhite:0 alpha:.5];
...
- (CGRect)handleBorderOverflow:(CGRect)newFrame {
    ...
    if (CGRectGetMaxX(newFrame) < CGRectGetMaxX(self.cropFrame)) newFrame.origin.x += CGRectGetMaxX(self.cropFrame) - CGRectGetMaxX(newFrame);
    // vertically
    if (newFrame.origin.y > self.cropFrame.origin.y) newFrame.origin.y = self.cropFrame.origin.y;
    if (CGRectGetMaxY(newFrame) < CGRectGetMaxY(self.cropFrame)) {
        newFrame.origin.y += CGRectGetMaxY(self.cropFrame) - CGRectGetMaxY(newFrame);
    }
