...
- (void)setupToolbars
{
    ...
    indexLabel.bounds = CGRectMake(0, 0,kScreenWidth, 30);
    ...
    if(self.timeString){
        indexLabel.text = [NSString stringWithFormat:@"%@  1/%ld",self.timeString,(long)self.imageCount];
    }else{
    ...
    saveButton.hidden = self.hideSaveButton;
...
- (void)photoClick:(UITapGestureRecognizer *)recognizer
{
    ...
    if ([self.sourceImagesContainerView isKindOfClass:UICollectionView.class]) {
        ...
        NSIndexPath *path = [NSIndexPath indexPathForItem:currentIndex inSection:self.indexPath.section];
...
- (void)showFirstImage
{
    ...
    if ([self.sourceImagesContainerView isKindOfClass:UICollectionView.class]) {
        UICollectionView *view = (UICollectionView *)self.sourceImagesContainerView;
        NSIndexPath *path = [NSIndexPath indexPathForItem:self.currentImageIndex inSection:self.indexPath.section];
        sourceView = [view cellForItemAtIndexPath:path];
    }else {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (!_willDisappear) {
        if(self.timeString){
            _indexLabel.text = [NSString stringWithFormat:@"%@  %d/%ld",self.timeString, index + 1, (long)self.imageCount];
        }else{
    ...
    if([self.sourceImagesContainerView isKindOfClass:[UICollectionView class]]){
        NSIndexPath *path = [NSIndexPath indexPathForItem:index inSection:self.indexPath.section];
